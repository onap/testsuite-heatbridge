...
class AAIManager:
    """AAIManager manages the connection state and interaction between an aai
       instance and the heatbridge."""
    ...
    def create_vserver_put(self, server_info_dict, volumes_dict):
        ...
        # required fields
        put['uri'] = ("/cloud-infrastructure/cloud-regions/cloud-region/"
                      + self.openstack_context.owner + "/"
                      + self.openstack_context.region + "/tenants/tenant/"
                      + self.openstack_context.tenant + "/vservers/vserver/"
                      + server_info_dict['id'])
        ...
        body['vserver-selflink'] = self.get_link(
            server_info_dict['links'], "self")
        # body['in-maint']
        ...
        if self.__exists(server_info_dict['metadata'], 'vnf_id'):
            data = self.__create_relationship_data(
                "generic-vnf", "vnf-id",
                server_info_dict['metadata']['vnf_id'])
            relation_list = self.__create_relationship_data_list(data)
            relations.append(
                self.__create_relationship("generic-vnf", relation_list))
            if self.__exists(server_info_dict['metadata'], 'vf_module_id'):
                data = self.__create_relationship_data(
                    "vf-module", "vf-module-id",
                    server_info_dict['metadata']['vf_module_id'])
                data2 = self.__create_relationship_data(
                    "generic-vnf", "vnf-id",
                    server_info_dict['metadata']['vnf_id'])
                relation_list = self.__create_relationship_data_list(
                    data, data2)
                relations.append(self.__create_relationship(
                    "vf-module", relation_list))
        if self.__exists(server_info_dict['flavor'], 'id'):
            data = self.__create_relationship_data(
                "flavor", "flavor-id", server_info_dict['flavor']['id'])
            data2 = self.__create_relationship_data(
                "cloud-region", "cloud-owner", self.openstack_context.owner)
            data3 = self.__create_relationship_data(
                "cloud-region", "cloud-region-id",
                self.openstack_context.region)
            relation_list = self.__create_relationship_data_list(
                data, data2, data3)
            relations.append(self.__create_relationship(
                "flavor", relation_list))
        if self.__exists(server_info_dict['image'], 'id'):
            data = self.__create_relationship_data(
                "image", "image-id", server_info_dict['image']['id'])
            data2 = self.__create_relationship_data(
                "cloud-region", "cloud-owner", self.openstack_context.owner)
            data3 = self.__create_relationship_data(
                "cloud-region", "cloud-region-id",
        ...
        data = self.__create_relationship_data(
            "vnfc", "vnfc-name", server_info_dict['name'])
        relation_list = self.__create_relationship_data_list(data)
        relations.append(self.__create_relationship("vnfc", relation_list))
    ...
    def create_flavor_put(self, flavor_dict):
        ...
        put['uri'] = ("/cloud-infrastructure/cloud-regions/cloud-region/"
                      + self.openstack_context.owner + "/"
                      + self.openstack_context.region + "/flavors/flavor/"
                      + flavor_dict['id'])
    ...
    def create_image_put(self, image_dict, server_info):
        ...
        put['uri'] = ("/cloud-infrastructure/cloud-regions/cloud-region/"
                      + self.openstack_context.owner + "/"
                      + self.openstack_context.region + "/images/image/"
                      + image_dict['id'])
        body['image-id'] = image_dict['id']
        body['image-name'] = image_dict['name']
        body['image-selflink'] = self.get_link(
            server_info['image']['links'], "bookmark")
        if self.__exists(image_dict, 'org.openstack__1__architecture'):
            body['image-architecture'] = (
                image_dict['org.openstack__1__architecture'])
        else:
    ...
    def create_l_interface_put(self, port_dict, server_info_dict):
        ...
        put['uri'] = ("/cloud-infrastructure/cloud-regions/cloud-region/"
                      ...
                      + self.openstack_context.region + "/tenants/tenant/"
                      + self.openstack_context.tenant + "/vservers/vserver/"
                      + port['device_id'] + "/l-interfaces/l-interface/"
                      + port['name'])
        ...
        for ips in port['fixed_ips']:
            if '.' in ips['ip_address']:
                v4list.append(
                    self.create_l3_interface_ipv4_address(port, ips))
            if ':' in ips['ip_address']:
                v6list.append(
                    self.create_l3_interface_ipv6_address(port, ips))
